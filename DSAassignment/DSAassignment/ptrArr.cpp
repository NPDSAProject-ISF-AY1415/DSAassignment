...
namespace ptrarr {
	...
	*/
	static inline void loadbar(unsigned int x, unsigned int n, clock_t beginClock, unsigned int w)
	{
		...
		std::cout << setw(3) << white << "Parsed: " << cyan << x << white << "/" << green << n << yellow << " [" << red;
		for (int x = 0; x < c; x++) std::cout << "=";
		for (int x = c; x < w; x++) std::cout << " ";
		std::cout << yellow << "] " << (int)(ratio * 100) << "%" << white << " Time Elapsed: " << cyan << setprecision(2) << fixed << elapsedSec;
		std::cout << " sec\r" << flush;
		eku::settextcolor(white);
	}
	...
	*/
	void parseFiles(ListPointer &musInfoList, ListPointer &wordList, ListPointer &lyricList){
		std::cout << pink << "How many lines to read in Music File? (-1 to read all): ";
		eku::settextcolor(cyan);
		int count;
		cin >> count;
		printSeperator();
